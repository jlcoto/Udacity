...
subset(ethqk, TOTAL_HOUSES_DAMAGED_DESCRIPTION > 4 & 
         !is.na(ethqk$TOTAL_HOUSES_DAMAGED_DESCRIPTION))

#Recoding case appropriately
ethqk[(ethqk$TOTAL_HOUSES_DAMAGED_DESCRIPTION > 4)  & 
        !is.na(ethqk$TOTAL_HOUSES_DAMAGED_DESCRIPTION),
      "TOTAL_HOUSES_DAMAGED_DESCRIPTION"] <- NA
```
...
plot_his_bins <- function(bins=30){
  plot_hist <- ggplot(aes(EQ_PRIMARY), 
                      data = subset(ethqk, !is.na(ethqk$EQ_PRIMARY))) +
               geom_histogram(bins=bins, colour= I("#144121"), 
                              fill = I("#0B8258")) +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
double_plot <- data.frame(ethqk$DEATHS,ethqk$TOTAL_DEATHS) %>% 
                                                        gather(var_orig, value)

ggplot(aes(x = value), 
       data = subset(double_plot, !is.na(double_plot$value))) +
       geom_histogram(aes(colour = "DEATHS"), 
                      data=subset(double_plot, 
                                  (double_plot$var_orig =="ethqk.DEATHS")  & 
                                    (!is.na(var_orig))), fill = I("#305649")) +
       geom_histogram(aes(colour = "TOTAL_DEATHS"),
                      data=subset(double_plot, 
                                  (double_plot$var_orig =="ethqk.TOTAL_DEATHS")  & 
                                  (!is.na(var_orig))), fill = I("#76D8B5")) +
       scale_colour_manual("Variable", values = c(I("#305649"), I("#76D8B5"))) +
       scale_x_log10() +
  ggtitle("Histogram of Deaths and Total Deaths")
...
## Checking other variables' distributions
...
ggplot(aes(cap_gdp), data = subset(ethqk, !is.na(ethqk$cap_gdp))) +
  geom_histogram() +
  scale_x_log10() +
  ggtitle("Histogram of Log GDP")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#Plotting number of tsunamis caused by an earthquake

ggplot(aes(x = tsun, y = ..count..), data = subset(ethqk, !is.na(ethqk$tsun))) +
  geom_bar() +
  ggtitle("Count of Earthquakes per Tsunami condition")
...
ggplot(aes(x = democ_autoc, fill = democ_autoc), 
       data = subset(ethqk, !is.na(democ_autoc))) +
  geom_bar() +
  ggtitle("Count of regimes per type") +
  theme(legend.position="none")
  
ggplot(aes(x = regime_type_4, fill = regime_type_4), 
       data = subset(ethqk, !is.na(democ_autoc))) +
  geom_bar() +
  ggtitle("Count of regimes per type") +
  theme(legend.position="none") 
...
ggplot(aes(x = count), 
       data = num_country) +
  geom_histogram() +
  scale_x_continuous(breaks = seq(0, 315, 20))
  ggtitle("Histogram of number of countries")
...
graph_lab <- c("1900-1913", "1914-1926", "1927-1939", "1940-1952", 
               "1953-1965", "1966-1978", "1979-1991", "1992-2004", "2005-2016")

p1 <- ggplot(aes(x=year, y=num_earth, color = countries, group = countries), 
             data = top_10_count) +
      geom_point() +
      geom_line() +
...
p2 <- ggplot(aes(x=year, y=num_earth, color = countries, group = countries), 
             data = ran_sample_count) +
      geom_point() +
      geom_line() +
      ylab("Number of Earthquakes") +
      scale_x_discrete(labels=graph_lab) +
...
scatter_ethqk_deaths + geom_point(alpha = 0.3, position = position_jitter(h=0))
```
...
#Only visualizing for earthquakes with less than 100 casualties
...
```{r, echo = FALSE, warning=FALSE, message=FALSE}
#Plotting boxplot
ggplot(aes(x = DEATHS_DESCRIPTION, y = EQ_PRIMARY), 
       data = subset(ethqk, DEATHS_DESCRIPTION != "None" 
                     & !is.na(DEATHS_DESCRIPTION))) +
...
#Plotting boxplot
ggplot(aes( x = tsun, y = EQ_PRIMARY), 
       data = subset(ethqk, !is.na(ethqk$EQ_PRIMARY))) +
...
ggplot(aes(x = democ_autoc, y = DEATHS), 
       data = subset(ethqk, !is.na(ethqk$democ_autoc))) +
...
ggplot(aes(x = regime_type_4, y = DEATHS), 
       data = subset(ethqk, !is.na(ethqk$regime_type_4))) +
...
ggplot(aes(x = regime_type_4, y = DEATHS, group = 1), 
       data = subset(ethqk, !is.na(ethqk$regime_type_4) & !is.na(ethqk$DEATHS) )) +
  scale_y_log10(labels = comma ) +
  geom_point(aes(colour = regime_type_4), alpha = 0.5) +
  geom_point(stat = "summary", fun.y = mean, 
             colour = I("#B0CE20"), size = 3) +
  geom_line(stat = "summary", fun.y = mean, 
            colour = I("#B0CE20"), size = 1.1) +
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#Plotting boxplot
ggplot(aes(x = democ_autoc, y = EQ_PRIMARY), 
       data = subset(ethqk, !is.na(ethqk$democ_autoc))) +
  geom_boxplot(aes(fill = democ_autoc )) +
...
ggplot(aes(x = regime_type_4, y = EQ_PRIMARY), 
       data = subset(ethqk, !is.na(ethqk$regime_type_4))) +
  geom_boxplot(aes(fill = regime_type_4 )) +
  ggtitle("Magnitude of Earthquake by regime type") +
  ylab("Magnitude") +
...
ggplot(aes(x = eq_source, y = ..count../sum(..count..), 
           fill = democ_autoc), 
       data = subset(ethqk, !is.na(ethqk$eq_source) & !is.na(ethqk$democ_autoc))) +
...
ggplot(aes(x = eq_source, y = ..count../sum(..count..), 
           fill = regime_type_4), 
       data = subset(ethqk, !is.na(ethqk$eq_source) & !is.na(ethqk$regime_type_4))) +
  geom_bar(position = "dodge") +
  ggtitle("Percentage of Type of Magnitude per regime type") +
  ylab("Percantage") +
  xlab("Source of magnitude")
...
ggplot(aes(x = 1*round(EQ_PRIMARY/1), y = DEATHS ),
       data = subset(ethqk, !is.na(tsun))) +
  geom_point(aes(colour = tsun),alpha=0.7, position = position_jitter(h = 0)) +
  scale_y_log10() +
...
ggplot(aes(x = EQ_PRIMARY, y = DEATHS), 
       data = subset(ethqk, !is.na(cap_gdp))) +
  geom_point(aes(colour = cap_gdp), alpha=0.5, 
             position = position_jitter(h = 0)) +
  scale_y_log10(labels = comma) +
  scale_colour_gradient(limits=c(400, 50000), space = "Lab") +
  ggtitle("Earthuakes Magnitude vs. Death rate and GDP")

range(ethqk$cap_gdp, na.rm = TRUE)
```
...
ggplot(aes(x = 0.5*round(EQ_PRIMARY/0.5), y = DEATHS ), 
       data = subset(ethqk, !is.na(rgdpe))) +
  geom_point(alpha=0.7, position = position_jitter(h = 0)) +
  scale_y_log10() +
  coord_cartesian(xlim = c(4.3, 7.6)) +
  geom_smooth(aes(color = gdp_class), stat = "summary", fun.y = mean, size = 1.5) +
...
p1_democ_autoc <- ggplot(aes(x = EQ_PRIMARY, y = DEATHS), 
                         data = subset(ethqk, !is.na(democ_autoc))) +
  geom_point(aes(colour = democ_autoc), alpha=0.7, 
             position = position_jitter(h = 0)) +
  scale_y_log10(labels = comma) +
  ggtitle("Earthuakes Magnitude vs. Deaths and type of regime") 


p2_regime_4 <- ggplot(aes(x = EQ_PRIMARY, y = DEATHS), 
                      data = subset(ethqk, !is.na(regime_type_4))) +
  geom_point(aes(colour = regime_type_4), alpha=0.7, 
             position = position_jitter(h = 0)) +
  scale_y_log10(labels = comma) +
  ggtitle("Earthuakes Magnitude vs. Deaths and type of regime")
...
ggplot(aes(x = 0.5*round(EQ_PRIMARY/0.5), y = DEATHS), 
  ...
  scale_y_log10() +
  coord_cartesian(xlim = c(4.3, 7.6)) +
  geom_smooth(aes(color = regime_type_4), 
              stat = "summary", fun.y = mean, size=1.5) +
